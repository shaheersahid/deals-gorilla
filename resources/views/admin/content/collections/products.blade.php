...
@section('page-title', 'Collection Products: ' . $collection->name)
...
@push('admin-styles')
    ...
    <style>
        #collection-products-table tbody tr { cursor: move; }
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title mb-0">Sort Products: {{ $collection->name }}</h4>
                            <p class="text-muted mb-0">Drag and drop rows to reorder products in this collection.</p>
                        </div>
                    ...
                    <div class="card-body">
                        <table id="collection-products-table" class="table table-hover table-bordered table-striped nowrap" style="width: 100%;">
...
<script>
$(function() {
    var table = $('#collection-products-table').DataTable({
        processing: true,
        serverSide: false, // Essential for RowReorder to be stable with client-side drag
        ajax: "{{ route('collections.products', $collection) }}",
        rowReorder: {
            selector: 'td.reorder-handle',
            dataSrc: 'sort_order', // This must match the column data name
            update: true
        },
        columns: [
            { 
        ...
        order: [[4, 'asc']], // Sort by sort_order column
        pageLength: 50
    });
    ...
    $('#save-order').on('click', function() {
        ...
        $('#collection-products-table tbody tr').each(function(index) {
            var rowData = table.row(this).data();
            if (rowData) {
                newOrder.push({
                    id: rowData.id,
                    position: index // 0-indexed position
                });
            }
        });
